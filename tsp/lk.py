...
class LinKernighanSolver:
    ...
    def solve(self, initial_tour: Optional[List[int]] = None) -> Tuple[List[int], float]:
        ...
        while improved:
            ...
            for t1 in t1_order:
                ...
                if new_tour is not None and tour_length(new_tour, self.dist) < tour_length(tour, self.dist):
                    tour = new_tour
                    improved = True
                    break
    ...
    def _search(
        ...
        used_x: Set[Tuple[int, int]],
        used_y: Set[Tuple[int, int]],
        depth: int,
    ) -> Optional[List[int]]:
        current_edges = (base_edges - removed) | added
        neighbors = adjacency_list(self.n, current_edges)
        ...
        for t_next in neighbors[last_vertex]:
            ...
            if t_next != t1 and closing_edge not in current_edges:
                ...
                if single_tour_check(self.n, candidate_edges):
                    ...
                    if candidate_tour is not None and tour_length(candidate_tour, self.dist) < tour_length(base_tour, self.dist):
            ...
            for t_new in self.candidates[t_next]:
                ...
                if t_new == t1 or yi in current_edges or yi in used_x2:
                    continue
